...
public class IncrementLine {
    ...
    public boolean isIncrementLineFull(){
        ...
        private static int randomBlock(int level) {
            ...
            if (ran < 20 && (level >0||!levelMode)) { return BLACK_BLOCK; } //2 %      >5
            else if (ran <35 && (level >0||!levelMode)) { return REPAINT_BLOCK; } //1,5%      >3
            else if (ran < 50 && (level >0||!levelMode)) { return REMOVE_SQUARE; } //1,5%  >4
            else if (ran< 55 && (level >0||!levelMode) ){ return REMOVER_BLOCK;} //0,5 > 2
